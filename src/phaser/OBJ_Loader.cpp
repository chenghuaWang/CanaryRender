...
bool objl::Vector2::operator!=(const objl::Vector2 &other) const {
    return !(this->X == other.X && this->Y == other.Y);
}

objl::Vector2 objl::Vector2::operator+(const objl::Vector2 &right) const {
...
bool objl::Vector3::operator==(const objl::Vector3 &other) const {
...
objl::Vector3 objl::Vector3::operator+(const objl::Vector3 &right) const {
...
objl::Mesh::Mesh(std::vector<Vertex> &_Vertices, std::vector<unsigned int> &_Indices) {
    Vertices = _Vertices;
    Indices = _Indices;
}
...
void objl::Loader::GenVerticesFromRawOBJ(std::vector<Vertex> &oVerts, const std::vector<Vector3> &iPositions,
